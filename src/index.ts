...
export function createWorker(
  locks: LockService = new MockLockService(),
  discordFetch: typeof fetch = fetch,
  cloudflareFetch: typeof fetch = fetch,
) {
  return {
    async fetch(request: Request, env: Env): Promise<Response> {
      ...
      if (path === "/install/account" && request.method === "GET") {
        ...
        if (!session) return Response.redirect(`${new URL(request.url).origin}/install`, 302);
        return accountSelectionPage(session.accounts, session.nonce);
      }
      if (path === "/install/account" && request.method === "POST") {
        const origin = request.headers.get("Origin");
        if (origin && origin !== new URL(request.url).origin) return installerErrorPage("不正な送信元です。", 403);
        const session = await readInstallSession(request, env);
